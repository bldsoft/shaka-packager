...
bool DvbTeletextParser::TeletextPage::is_empty() noexcept {
  ...
  for (auto col = 0; col < kColumnsCount; col++) {
    for (auto row = 1; row < kRowsCount; row++) {
...
void DvbTeletextParser::ProcessTeletextPage(
  ...
  // Process page data.
  for (int row = 1; row < TeletextPage::kRowsCount; row++) {
    line.clear();
    line_utf8.clear();
    ...
    for (auto col = colStart + 1; col < TeletextPage::kColumnsCount; col++) {
  ...
  /*
      for (auto sample : samples) {
          sample->set_sub_stream_index(frame.page());
