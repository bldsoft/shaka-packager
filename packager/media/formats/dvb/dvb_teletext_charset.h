...
#ifndef PACKAGER_MEDIA_DVB_DVB_TELETEXT_CHARSET_H_
#define PACKAGER_MEDIA_DVB_DVB_TELETEXT_CHARSET_H_
...
class DvbTeletextCharset {
 public:
  ...
  //!
  char16_t teletextToUcs2(uint8_t c) const;
  ...
  //! @return UCS-2 equivalent.
  //!
  char16_t g2AccentToUcs2(uint8_t c, uint8_t accent) const;
  ...
  //! @return UCS-2 equivalent.
  //!
  char16_t g2ToUcs2(uint8_t c) const;
  ...
  //!
  //! Set the M/29 character set and use it as current character set if X/28 is
  //! not defined.
  //! @param [in] charset Charactet set index.
  //!
 ...
 private:
  ...
  //! G0 charsets data.
  //!
  typedef char16_t G0CharsetData[G0_COUNT][CHAR_COUNT];
